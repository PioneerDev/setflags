CREATE TABLE IF NOT EXISTS flags (
  ...
  payer_id          VARCHAR(36) NOT NULL CHECK (user_id ~* '^[0-9a-f-]{36,36}$'),
...
CREATE TABLE IF NOT EXISTS evidences (
  attachement_id        VARCHAR(36) PRIMARY KEY CHECK (message_id ~* '^[0-9a-f-]{36,36}$'),
  flag_id               VARCHAR(36) NOT NULL CHECK (conversation_id ~* '^[0-9a-f-]{36,36}$'),
  file                  TEXT NOT NULL,
  type              	VARCHAR(512) NOT NULL,
  created_at            TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  PRIMARY KEY(attachment_id, flag_id)
);
...
CREATE TABLE IF NOT EXISTS witnesses (
  ...
  payee_id           VARCHAR(36) NOT NULL CHECK (user_id ~* '^[0-9a-f-]{36,36}$'),
  created_at        TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  amount            VARCHAR(128) NOT NULL,
